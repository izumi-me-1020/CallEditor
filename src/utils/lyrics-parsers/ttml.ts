...
const COMPOSER_NS = "https://calleditor.izumy.me/ttml";
...
function parseTtml(content: string, _fallbackDuration?: number): ParseResult {
  ...
  // Parse calleditor:groups registry
  const groups: LinkGroup[] = [];
  const groupEls = Array.from(
    doc.getElementsByTagName("calleditor:group"),
  ...
  for (const p of paragraphs) {
    ...
    // Extract calleditor: group attrs (try plain attribute first, then namespaced lookup)
    const rawGroupId =
      p.getAttribute("calleditor:groupId") ??
    ...
    if (rawGroupId && !knownGroupId) {
      console.warn(
        `[CallEditor] TTML <p> references unknown groupId="${rawGroupId}"; treating line as standalone.`,
      );
    }
    const instanceIdxStr =
      p.getAttribute("calleditor:instanceIdx") ??
      p.getAttributeNS(COMPOSER_NS, "instanceIdx") ??
      null;
    const templateLineIdxStr =
      p.getAttribute("calleditor:templateLineIdx") ??
      p.getAttributeNS(COMPOSER_NS, "templateLineIdx") ??
      null;
    const detachedStr =
      p.getAttribute("calleditor:detached") ??
