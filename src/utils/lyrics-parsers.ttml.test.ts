...
describe("parseLyricsFile - TTML word explicit attribute", () => {
  ...
  it('recognizes calleditor:explicit="true" on a word span', () => {
    const content = `<tt xmlns="http://www.w3.org/ns/ttml" xmlns:ttm="http://www.w3.org/ns/ttml#metadata" xmlns:calleditor="https://calleditor.izumy.me/ttml"><head><metadata><ttm:agent type="person" xml:id="v1"/></metadata></head><body><div><p begin="00:01.000" end="00:02.000" ttm:agent="v1"><span begin="00:01.000" end="00:01.500">clean</span> <span begin="00:01.500" end="00:02.000" calleditor:explicit="true">dirty</span></p></div></body></tt>`;
    const result = parseLyricsFile("song.ttml", content);
    const words = result.lines[0].words!;
    expect(words[0].explicit).toBeUndefined();
  ...
  it('treats explicit="false" / "0" as not explicit', () => {
    const content = `<tt xmlns="http://www.w3.org/ns/ttml" xmlns:ttm="http://www.w3.org/ns/ttml#metadata" xmlns:calleditor="https://calleditor.izumy.me/ttml"><head><metadata><ttm:agent type="person" xml:id="v1"/></metadata></head><body><div><p begin="00:01.000" end="00:02.000" ttm:agent="v1"><span begin="00:01.000" end="00:01.500" calleditor:explicit="false">clean</span> <span begin="00:01.500" end="00:02.000" calleditor:explicit="0">also</span></p></div></body></tt>`;
    const result = parseLyricsFile("song.ttml", content);
    const words = result.lines[0].words!;
    expect(words[0].explicit).toBeUndefined();
  ...
  it("recognizes explicit on a word inside x-bg, landing on backgroundWords", () => {
    const content = `<tt xmlns="http://www.w3.org/ns/ttml" xmlns:ttm="http://www.w3.org/ns/ttml#metadata" xmlns:calleditor="https://calleditor.izumy.me/ttml"><head><metadata><ttm:agent type="person" xml:id="v1"/></metadata></head><body><div><p begin="00:01.000" end="00:02.500" ttm:agent="v1"><span begin="00:01.000" end="00:02.000">main</span><span ttm:role="x-bg"><span begin="00:02.000" end="00:02.250">oh</span> <span begin="00:02.250" end="00:02.500" calleditor:explicit="true">shit</span></span></p></div></body></tt>`;
    const result = parseLyricsFile("song.ttml", content);
