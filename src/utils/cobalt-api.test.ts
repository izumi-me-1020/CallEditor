...
describe("mapError - known codes", () => {
  ...
  it("maps forbidden_origin to a user-readable message", () => {
    expect(mapError("forbidden_origin")).toBe(
      "This site is not allowed to make this request",
    );
  });
...
describe("formatCobaltErrorForToast", () => {
  const defaultCtx = { isDefault: true, instanceLabel: "CallEditor" };
  ...
  it("explains turnstile_unconfigured with setup guidance", () => {
    const msg = formatCobaltErrorForToast(
      new CobaltApiError("turnstile_unconfigured", 0),
      defaultCtx,
    );
    expect(msg).toContain("VITE_TURNSTILE_SITEKEY");
  });

  it("explains forbidden_origin as an origin restriction", () => {
    const msg = formatCobaltErrorForToast(
      new CobaltApiError("forbidden_origin", 403),
      defaultCtx,
    );
    expect(msg.toLowerCase()).toContain(
      "doesn't allow requests from this site",
    );
  });
