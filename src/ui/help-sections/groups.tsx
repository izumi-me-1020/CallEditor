...
const COPY = {
  ...
  ja: {
    ...
    why: "グループ化する理由",
    whyBody:
      "同じサビが 4 回出る曲なら、通常はコール修正を 4 箇所に入れる必要があります。グループ化すれば 1 箇所の修正が全部に反映されます。音節分割、単語のバックボーカル化、エージェント変更も同じです。",
    create: "グループの作成",
    createBodyA: "",
    createBodyB:
    ...
    addList: [
      ...
      `playhead が再生中の行の中にある、隙間が足りない、最後のコールを通り過ぎている場合はクリップボードへコピーされ、貼り付けゴーストが開きます。トーストには「No room at the playhead. ${MOD_KEY} + V to paste somewhere clear.」のように次の案内が出ます。`,
    ],
  ...
  ko: {
    ...
    why: "왜 그룹을 쓰나",
    whyBody:
      "후렴이 네 번 반복되는 곡이라면 원래는 같은 콜 수정을 네 군데에 해야 합니다. 그룹화하면 한 곳의 수정이 전부에 반영됩니다. 음절 분할, 단어를 백보컬로 바꾸는 것, 에이전트 재할당도 같습니다.",
    create: "그룹 만들기",
    createBodyA: "",
    createBodyB:
    ...
    addList: [
      ...
      `playhead가 재생 중인 줄 안에 있거나 간격이 부족하거나 마지막 콜을 이미 지나쳤다면 인스턴스를 클립보드에 복사하고 붙여넣기 미리보기를 엽니다. 토스트에는 “No room at the playhead. ${MOD_KEY} + V to paste somewhere clear.” 같은 안내가 뜹니다.`,
    ],
