...
describe("TimelineSection", () => {
  it("renders the section content", async () => {
    ...
    await expect
      .element(screen.getByRole("heading", { name: "Layout" }))
      .toBeInTheDocument();
  });
  ...
  it("documents explicit-word marking and detection", async () => {
    const screen = await render(<TimelineSection />);
    await expect
      .element(screen.getByRole("heading", { name: "Explicit words" }))
      .toBeInTheDocument();
    await expect
      .element(screen.getByText(/calleditor:explicit/))
      .toBeInTheDocument();
  });
  ...
  it("does not list a stale Select toolbar entry", async () => {
    ...
    expect(screen.container.textContent).not.toContain(
      "disables double-click word creation",
    );
  });
  ...
  it("documents wheel-over-waveform scrubbing", async () => {
    ...
    await expect
      .element(
        screen.getByText(
          /Scroll the wheel while the cursor is over the waveform strip/,
        ),
      )
      .toBeInTheDocument();
  });
  ...
  it("documents the scroll wheel timeline setting", async () => {
    ...
    await expect
      .element(
        screen.getByText(/Turn on "Scroll wheel scrolls timeline" in Settings/),
      )
      .toBeInTheDocument();
  });
  ...
  it("documents playhead-drag edge auto-scroll", async () => {
    const screen = await render(<TimelineSection />);
    await expect
      .element(screen.getByText(/auto-scrolls the view/i))
      .toBeInTheDocument();
  });
  ...
  it("documents selecting the word under the playhead", async () => {
    const screen = await render(<TimelineSection />);
    await expect
      .element(screen.getByText(/cycle through any overlapping words/i))
      .toBeInTheDocument();
  });
