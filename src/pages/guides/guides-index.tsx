...
const PATH = "/guides";
const TITLE = "TTML and Synced Lyrics Guides ・ CallEditor";
const DESCRIPTION =
  ...
  return (
    <LandingLayout>
      ...
      <section className="px-6 py-16 md:py-24 max-w-4xl mx-auto">
        <p className="text-xs font-medium tracking-wide text-calleditor-accent-text mb-4 select-none">
          Guides
        </p>
        <h1 className="text-3xl md:text-5xl font-semibold text-calleditor-text mb-5">
          Learn TTML and synced lyrics authoring.
        </h1>
        <p className="text-lg text-calleditor-text-secondary max-w-2xl">
      ...
      <section className="px-6 pb-20 max-w-4xl mx-auto">
        <ul className="divide-y divide-calleditor-border border-y border-calleditor-border">
          {GUIDES.map((guide) => (
            <li key={guide.slug}>
              <Link
                to={`/guides/${guide.slug}`}
                className="group flex items-center justify-between gap-5 py-5 hover:bg-calleditor-bg-elevated/50 transition-colors px-2 -mx-2 rounded-lg"
              >
                <div className="flex-1 min-w-0">
                  <h2 className="text-lg font-semibold text-calleditor-text mb-1">
                    {guide.title}
                  </h2>
                ...
                <IconArrowRight
                  size={18}
                  className="text-calleditor-text-muted group-hover:text-calleditor-accent-text transition-colors flex-shrink-0"
                />
              </Link>
            </li>
          ))}
