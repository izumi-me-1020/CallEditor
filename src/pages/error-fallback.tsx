...
const ErrorFallback: React.FC = () => {
  ...
  const recoveryMessage =
    recoveryStatus === "success"
      ? "Saved. Open CallEditor, head to the Export tab, and click Import Project to keep going."
  ...
  return (
    <div className="min-h-screen bg-calleditor-bg text-calleditor-text flex items-center justify-center p-6 select-none">
      <div className="w-full max-w-lg flex flex-col items-center text-center gap-5">
        <Icon
          size={56}
          strokeWidth={1.5}
          className="text-calleditor-text opacity-50"
        />

        <div className="flex flex-col gap-1.5">
          <h1 className="text-2xl font-semibold text-calleditor-text">
            {details.title}
          </h1>
          {details.errorName && details.errorName !== "Error" && (
            <p className="text-xs font-mono text-calleditor-text-muted select-text">
              {details.errorName}
            </p>
          )}
          <p className="text-sm text-calleditor-text-secondary leading-relaxed select-text break-words">
            {details.subtitle}
          </p>
        </div>
        ...
        {recoveryStatus === "success" && (
          <ClearRecoveryButton clearedMessage="Cleared. Reload CallEditor to start fresh." />
        )}
        ...
        {hasDetails && (
          <div className="w-full mt-2 flex flex-col items-center gap-2">
            <button
              ...
              className="inline-flex items-center gap-1 text-xs text-calleditor-text-muted hover:text-calleditor-text transition-colors cursor-pointer"
            ...
            {showDetails && (
              <div className="w-full flex flex-col gap-2 text-left">
                {responseDataString && (
                  <Scroll className="rounded-md bg-calleditor-button max-h-48">
                    <pre className="p-3 text-[11px] leading-relaxed text-calleditor-text-secondary select-text font-mono">
                      {responseDataString}
                    </pre>
                  </Scroll>
                )}
                {details.stack && (
                  <Scroll className="rounded-md bg-calleditor-button max-h-72">
                    <pre className="p-3 text-[11px] leading-relaxed text-calleditor-text-secondary select-text font-mono whitespace-pre-wrap">
                      {details.stack}
                    </pre>
                  </Scroll>
                )}
